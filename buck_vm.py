...
for vin in vin_list:
    ...
    # 补偿级 传递函数
    Gvc_num = (-wp0/s)*(1+s/wz_c)*(1+s/wz_ff)
    Gvc_den = (1+s/wp_ff)*(1+s/wp_hf)
    Gvc = Gvc_num/Gvc_den
    ...
    # 功率级传递函数 
    Gvd_num = 1 + s*R_ESR*C_O
    Gvd_den = 1 + s*(R_ESR*C_O + L_bk/R_O) + s**2*L_bk*C_O*(1+R_ESR/R_O)
    Gvd = -VIN*Gvd_num/Gvd_den
...
plot_bode(ax_mag, ax_ph,          
          xlim=(1, 1e7),                            # x轴范围
          mag_ylim=(-40, 100),                      # 幅度 y轴范围
          mag_yticks=[-20, 0, 20, 40, 60, 80, 100], # 幅度 y轴刻度
          phase_ylim=(-200, 10),                    # 相位 y轴范围          
